...
export function AuthProvider({ children }){
    ...
    const loginUserWithCredentials = async ( email, password ) => {
        try{
            const { data : { user, token, success, message }} = await axios.post(`https://crickart.herokuapp.com/login`,
        {
            email,
            password
        })
    ...
    const createUserWithCredentials = async ( name, email, password ) => {
       try{ 
           const { data : { user, token, success, message }} = await axios.post(`https://crickart.herokuapp.com/signup`,
        {
            name : name,
            email : email,
            password : password,
    ...
    return (
        <AuthContext.Provider value={{ user, setUser, loginUserWithCredentials, createUserWithCredentials, logout }}>
            {children}
