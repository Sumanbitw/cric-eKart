    ...
    <div className="wishlist__item-details">
      <div className="wishlist__list">
        ...
        <div className="wishlist__name">
          <span className="wishlist__item">
            {wishlistItem && wishlistItem.title}
          </span>
          <br />
          <div className="whislist__price__details">
            ...
            <p style={{ color: "green" }}>
              {" "}
              {wishlistItem && wishlistItem.discount} %
            </p>
            <p style={{ textDecoration: "line-through", color: "grey" }}>
              {" "}
              ₹ {wishlistItem && wishlistItem.netPrice}
            </p>
          </div>
          <br />
            ...
            {itemsInCart() ? (
              <Link to="/cart">
                <button className="wishlist__btn">
                  <i className="fa fa-shopping-cart"> Go to Bag </i>
                </button>
              </Link>
            ) : (
              <button className="wishlist__btn" onClick={addToCart}>
                <i className="fa fa-shopping-cart"> Add to Bag </i>
              </button>
            )}
            <ToastContainer />
            <button onClick={removeItem} className="wishlist__remove">
              Remove
            </button>
    ...
    (async function getItemsInCart() {
      try {
        const response = await axios.get(
          `https://crickart.herokuapp.com/wishlist/${user._id}`
        );
